...
from dataclasses import dataclass  # Pour simplifier la création de classes de données en Python avec la syntaxe des décorateurs, ce qui rend le code plus concis et lisible.
from typing import Optional, Dict, Any  # Pour annoter les types dans mon code, améliorant ainsi la lisibilité et permettant une meilleure compréhension du fonctionnement des fonctions et des méthodes.
from requests.exceptions import RequestException  # Pour capturer spécifiquement les exceptions liées aux requêtes HTTP, telles que les erreurs de connexion ou de délai d'attente.
